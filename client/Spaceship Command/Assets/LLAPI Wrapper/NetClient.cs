...
public class NetClient{
    ...
    public bool SendMessage( MessageBase msg) {
        NetworkWriter writter = new NetworkWriter();
        msg.Serialize(writter);

        byte error;
        NetworkTransport.Send ( mSocket , mConnection , NetManager.mChannelReliable , writter.AsArray() , writter.AsArray().Length , out error );

        if( NetUtils.IsNetworkError ( error )){
            Debug.Log("NetClient::SendStream() Failed with reason '" + NetUtils.GetNetworkError (error) + "'.");
            return false;
        }
